...
def test_a_water_command_rides_the_next_report_response(client, db):
    answer = command(client, "c=butler1 water=3 ml=50 cap_s=30")
    assert answer.status_code == 200
    assert answer.text == "cmd=1\n"

    answer = report(client, "c=butler1 t=1000 ch0=8000")
...
def test_the_following_report_acks_it_and_the_flow_count_is_kept(client, db):
    command(client, "c=butler1 water=3 ml=50 cap_s=30")
    report(client, "c=butler1 t=1000 ch0=8000")

    answer = report(client, "c=butler1 t=61000 ch0=8000 ack=1 flow_ml=48")
    assert answer.text == "next=60\n"
    with sqlite3.connect(db) as con:
...
def test_a_stop_command_is_handed_as_stop(client, db):
    command(client, "c=butler1 stop=1")
...
def test_a_command_waits_for_its_own_controller(client, db):
    command(client, "c=butler2 water=1 ml=50 cap_s=30")
...
def test_a_report_without_the_ack_expires_the_sent_command(client, db):
    command(client, "c=butler1 water=3 ml=50 cap_s=30")
    report(client, "c=butler1 t=1000 ch0=8000")  # handed here

    answer = report(client, "c=butler1 t=61000 ch0=8000")  # no ack
...
def test_a_lost_response_expires_the_command_it_carried(client, db):
    ...
    retry = report(client, "c=butler1 t=1000 ch0=8000")
...
def test_a_late_ack_for_an_expired_command_changes_nothing(client, db):
    command(client, "c=butler1 water=3 ml=50 cap_s=30")
    report(client, "c=butler1 t=1000 ch0=8000")
    report(client, "c=butler1 t=61000 ch0=8000")  # expires it

    report(client, "c=butler1 t=121000 ch0=8000 ack=1 flow_ml=48")
    assert states(db) == {1: "expired"}
...
def test_the_slot_holds_one_command_while_queued(client, db):
    ...
    answer = command(client, "c=butler1 stop=1")
...
def test_controllers_have_their_own_slots(client, db):
    command(client, "c=butler1 water=3 ml=50 cap_s=30")
    assert command(client, "c=butler2 water=1 ml=50 cap_s=30").status_code == 200


def test_an_abandoned_command_frees_the_slot_after_the_ttl(client, db):
    command(client, "c=butler1 water=3 ml=50 cap_s=30")
    report(client, "c=butler1 t=1000 ch0=8000")  # sent; the board dies now
    with sqlite3.connect(db) as con:
        con.execute(
    ...
    assert command(client, "c=butler1 stop=1").status_code == 200
...
def test_the_interval_knob_changes_what_reports_are_told(client, db):
    assert interval(client, "c=butler1 next=120").text == "next=120\n"
...
def test_next_0_clears_the_override(client, db):
    interval(client, "c=butler1 next=120")

    assert interval(client, "c=butler1 next=0").text == "next=60\n"
    assert report(client, "c=butler1 t=1000 ch0=8000").text == "next=60\n"
...
def test_health_shows_heartbeat_and_the_open_command(client, db):
    ...
    assert entry["controller"] == "butler1"
...
def test_health_lists_a_configured_but_never_seen_controller(client, db):
    interval(client, "c=ghost next=120")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "c=butler1",  # neither water= nor stop=1
        "c=butler1 water=3 ml=50 cap_s=30 stop=1",  # both at once
        ...
        "c=butler1 water=999 ml=50 cap_s=30",  # outlet out of range
        "c=butler1 water=3 water=4 ml=50 cap_s=30",  # water= twice
        "c= c=evil stop=1",  # empty c= must not open the door to a second
        "c=butler1 water=+3 ml=50 cap_s=30",  # leading +: refusal, not repair
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "c=butler1",  # no next=
        "c=butler1 next=4",  # below the floor
        "c=butler1 next=9999",  # above the ceiling
        "c=butler1 next=abc",  # not an integer
    ],
)
...
def test_the_knob_cannot_let_a_live_board_outlive_the_command_ttl(client, db):
    ...
    answer = interval(client, "c=butler1 next=3600")
    assert answer.status_code == 400
...
def test_commands_and_the_knob_need_the_token_too(client, db):
    assert command(client, "c=butler1 stop=1", token="nope").status_code == 401
    assert interval(client, "c=butler1 next=120", token="nope").status_code == 401
    assert states(db) == {}
...
def test_ack_parsing_is_as_strict_as_the_rest():
    r = parse_report("c=x ch0=1 ack=17 flow_ml=48")
    assert (r.ack, r.flow_ml) == (17, 48)
    for bad in [
        "c=x ch0=1 flow_ml=48",  # flow_ml without ack
        "c=x ch0=1 ack=0",  # command ids start at 1
        "c=x ch0=1 ack=17 ack=18",  # ack twice
        "c=x ch0=1 ack=nope",  # not an integer
    ]:
